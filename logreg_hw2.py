...
def main():
  ...
  w,_= trainLogistic(X_train_bias, y_train, best_iterations, best_step_size)
...
def calculateNegativeLogLikelihood(X,y,w):
  ...
  # z is the training data matrix multiplied by a weight vector
  z = X@w
...
def trainLogistic(X,y, max_iters=max_iters, step_size=step_size):
    ...
    for i in range(max_iters):
        ...
        p = logistic(z)
        # print(f"p shape: {np.shape(p)}")

        w_grad = np.matmul(X.T, (p - y))
        # print(f"w_grad shape before: {np.shape(w_grad)}")

        w_grad = w_grad / len(y)
        # print(f"w_grad shape after: {np.shape(w_grad)}")
...
######################################################################
def dummyAugment(X):
  # create 1 x N-cols of X and fill with ones
  bias_col = np.ones((X.shape[0], 1))

  # add new column of ones to the left hand side of X
  new_X = np.hstack((bias_col, X))
